function [img_files, pos, target_sz, ground_truth, video_path] = load_video_info_uav(base_path,base_path_anno, video)
%LOAD_VIDEO_INFO
...
%   http://www.isr.uc.pt/~henriques/
    ...
    base_path_anno=[base_path_anno video];
   

	%try to load ground truth from text file (Benchmark's format)
   % filename = [video_path 'groundtruth' suffix '.txt'];
	filename = [base_path_anno '_1.txt']
  	%filename = [video_path video '_gt' suffix '.txt'];
	f = fopen(filename);
...
%     if isempty(idx)
%        gtset(1)=1;
%        gtset(2)=size(ground_truth,1);
%     else
%        gtset(1)=specialframes{idx,2};
%         gtset(2)=specialframes{idx,3};
%     end
...
%           
%             img_files = num2str((gtset(1) : gtset(2))', '%04i.jpg');
%     
